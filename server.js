...
(async () => {
  try {
    ...
    console.error('Startup directory creation failed:', err);
    process.exit(1);
  }
})();

// Multer storage (save under public/uploads)
...
// Serve static client files (index.html, app.js, css) and the uploads folder
...
// API: create a new post (multipart/form-data) with optional file upload
// fields: title, author, tags, content, imageURL (optional), file field name: imageFile
app.post('/api/posts', upload.single('imageFile'), async (req, res, next) => {
  try {
    ...
    let image = null;
    if (req.file) {
      image = '/uploads/' + req.file.filename; // accessible via static server
...
app.use((err, req, res, next) => {
  ...
  if (err && err.code === 'LIMIT_FILE_SIZE') {
    return res.status(413).json({ error: 'File too large. Max size is 5 MB.' });
  }
